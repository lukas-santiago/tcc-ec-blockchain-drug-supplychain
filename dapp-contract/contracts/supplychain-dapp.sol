...
contract SupplyChainDApp is SupplyChainDAppBase {
    ...
    function grantOperatorRole(
        ...
        require(
            userCompanyOwner.companyIds.includes(companyId),
            "Company's owner is another."
        );
        require(
            userOperator.operatorIds.includes(companyId) == false,
    ...
    function getCompany(
        uint32 companyId
    ) public view returns (CompanyModel memory) {
    ...
    function disableCompany(
        uint32 companyId
    ) public onlyRole(COMPANY_ROLE) isCompanyOwner(companyId) {
    ...
    modifier isOperatorOfCompany(uint32 companyId) {
        UserModel storage user = users[msg.sender];
        bool checked = false;
        for (uint i = 0; i < user.operatorIds.length; i++)
            if (user.operatorIds[i] == companyId) checked = true;
    ...
    modifier isIntermediateCompany(uint32 companyId) {
        CompanyModel memory company = companies[companyId];
        require(
            company.isIntermediate,
            "Company is not an intermediate company."
        );
        _;
    }
    ...
    function getCatalog(
        uint32 catalogId
    ) public view returns (CatalogModel memory) {
    ...
    modifier onlyValidLot(uint32 lotId) {
        require(
            lots[lotId].lotId > 0 && lots[lotId].lotId <= lotCount,
            "invalid lot id"
        );
        _;
    }
    ...
    function registerActivity(
        ...
        onlyRole(OPERATOR_ROLE)
        isOperatorOfCompany(companyId)
        isIntermediateCompany(companyId)
        onlyValidLot(lotId)
    {
        activityCount++;
