...
contract ManufacturerCompany is Models {
    ...
    function editLot(
        ...
        onlyValidProducts(newProductIds)
    {
    ...
    modifier onlyUnconfirmedLot(uint16 lotId) {
        require(
            !lots[lotId].confirmed,
            "cannot modify or disable a confirmed lot"
        );
        _;
    }
    ...
    modifier onlyValidProducts(uint16[] memory productIds) {
        for (uint256 i = 0; i < productIds.length; i++) {
            require(
                products[productIds[i]].productId > 0 &&
                    products[productIds[i]].productId <= productCount,
                "invalid product id"
            );
        }
        _;
